...
function ReportView({ dims, selfScores, groups, comments, scaleMax: propScaleMax }) {
  ...
  const radarData = dims.map(d => {
    ...
    if (hasOthers) row['Others' Average'] = dimAvg(othersAvg, d);
    return row;
  });
  const barData = dims.map(d => ({
    name:d.id, label:d.label, color:d.color,
    'Self-Assessment': dimAvg(ss, d),
    ...(hasOthers ? {'Others' Average': dimAvg(othersAvg, d)} : {}),
  }));
              ...
              <div>
                ...
                <ResponsiveContainer width="100%" height={280}>
                  <RadarChart data={radarData} margin={{top:10,right:30,bottom:10,left:30}}>
                    ...
                    {hasOthers && <Radar name="Others' Average" dataKey="Others' Average" stroke={BLUE} fill={BLUE} fillOpacity={0.1} strokeWidth={2} strokeDasharray="4 2" dot={{fill:BLUE,r:3}}/>}
              ...
              <div>
                ...
                <ResponsiveContainer width="100%" height={280}>
                  <BarChart data={barData} layout="vertical" margin={{left:55}}>
                    ...
                    {hasOthers && <Bar dataKey="Others' Average" fill={BLUE} fillOpacity={0.45} radius={4}/>}
                  </BarChart>
                </ResponsiveContainer>
              </div>
            </div>
