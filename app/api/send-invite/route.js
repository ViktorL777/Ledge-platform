...
export async function POST(request) {
  ...
  try {
    const { to, raterName, code, surveyTitle, participantName, senderName, subject: customSubject, htmlBody: customHtml } = await request.json();
    if (!to || !code) {
      return Response.json(
    ...
    const { data, error } = await resend.emails.send({
      ...
      to: [to],
      subject: customSubject || `${surveyTitle || 'LEDGE 360° Assessment'} — Please complete your evaluation`,
      html: customHtml || `
