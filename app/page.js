...
// Supabase public client (read-only)
...
// ============================================
// DIMENSION CONFIG
// Maps display names to database dimension values
// ============================================
const DIMENSIONS = [
  {
    name: 'Meaning Maker',
    dbKey: 'Meaning-Maker',
    desc: 'Purpose, values, philosophy, ethics',
    icon: '◈',
  },
  {
    name: 'Strategy',
    dbKey: 'Strategist',
    desc: 'Business strategy, competitive moves',
    icon: '◇',
  },
  {
    name: 'New Technologies',
    dbKey: 'Tech-Savvy',
    desc: 'AI, robotics, quantum, biotech',
    icon: '⬡',
  },
  {
    name: 'Operations',
    dbKey: 'Operator',
    desc: 'Process innovation, automation',
    icon: '⊞',
  },
  {
    name: 'Interpersonal Relationships',
    dbKey: 'Relationship-Weaver',
    desc: 'Talent, interpersonal dynamics',
    icon: '◎',
  },
  {
    name: 'Culture',
    dbKey: 'Culture-Architect',
    desc: 'Org culture, employee experience',
    icon: '⊕',
  },
  {
    name: 'Self-Mastery',
    dbKey: 'Self-Awareness',
    desc: 'Resilience, mindset, growth',
    icon: '◉',
  },
  {
    name: 'Change & Transformation',
    dbKey: 'Transformator',
    desc: 'Transformation, M&A, disruption',
    icon: '⟐',
  },
];
...
export default function Home() {
  ...
  // Fetch latest articles grouped by dimension
  useEffect(() => {
    async function fetchArticles() {
      try {
        ...
        if (error) {
          ...
          if (allData) {
            const grouped = groupByDimension(allData);
            setArticles(grouped);
          }
        } else if (data) {
  ...
  function groupByDimension(data) {
    const grouped = {};
    for (const article of data) {
      const dim = article.primary_dimension;
      if (!grouped[dim]) grouped[dim] = [];
      grouped[dim].push(article);
    }
    return grouped;
  }
  ...
  const totalArticles = Object.values(articles).reduce((sum, arr) => sum + arr.length, 0);
  ...
  return (
    <>
      ...
      {/* ============================================ */}
      {/* NEWS DIMENSIONS — Dynamic articles section  */}
      {/* ============================================ */}
      <section className="dimensions-section" id="news">
        <div className="section-inner">
          ...
          <div className="section-text">
            ...
            {totalArticles > 0 && (
              <span className="article-count"> Currently tracking {totalArticles} articles.</span>
            )}
          </div>
          ...
          <div className="dimension-pills">
            <button
              className={`dim-pill ${activeDimension === null ? 'active' : ''}`}
              onClick={() => setActiveDimension(null)}
            >
              All dimensions
            </button>
            {DIMENSIONS.map((dim) => {
              const count = (articles[dim.dbKey] || []).length;
              return (
                <button
                  key={dim.dbKey}
                  className={`dim-pill ${activeDimension === dim.dbKey ? 'active' : ''}`}
                  onClick={() => setActiveDimension(
                    activeDimension === dim.dbKey ? null : dim.dbKey
                  )}
                >
                  {dim.name}
          ...
          {loading && (
            <div className="loading-state">
              ...
              <p>Loading leadership intelligence...</p>
            </div>
          )}

          {/* Dimension cards with articles */}
          {!loading && (
            <div className="dimensions-grid">
              {DIMENSIONS
                .filter(dim => activeDimension === null || activeDimension === dim.dbKey)
                .map((dim) => {
                  const dimArticles = (articles[dim.dbKey] || []).slice(0, 3);
                  return (
                    <div key={dim.dbKey} className="dimension-block">
                      {/* Dimension header */}
                      <div className="dim-header">
                        <span className="dim-icon">{dim.icon}</span>
                        <div>
                          <h3 className="dim-name">{dim.name}</h3>
                          <p className="dim-desc">{dim.desc}</p>
                        </div>
                      </div>

                      {/* Articles for this dimension */}
                      {dimArticles.length > 0 ? (
                        <div className="dim-articles">
                          {dimArticles.map((article) => (
                            <a
                              key={article.id}
                            ...
                            >
                              ...
                              {article.leadership_angle && (
                                <div className="leadership-angle">
                                  <div className="angle-label">Leadership Angle</div>
                                  <p className="angle-text">{article.leadership_angle}</p>
                                </div>
                              )}
                        ...
                        <div className="dim-empty">
                          <p>Articles coming soon for this dimension.</p>
                        </div>
                      )}

                      {/* View all link */}
                      {(articles[dim.dbKey] || []).length > 3 && (
                        <a href={`/news?dimension=${dim.dbKey}`} className="dim-view-all">
                          View all {(articles[dim.dbKey] || []).length} articles →
                        </a>
                      )}
                    </div>
                  );
          ...
          {!loading && totalArticles === 0 && (
            <div className="empty-state">
              ...
              <h3>The Daily Ledge is warming up.</h3>
              <p>Our AI is curating the first batch of leadership intelligence. Check back soon, or subscribe below to get notified when we launch.</p>
            </div>
          )}
        </div>
      </section>
      ...
      {/* ============================================ */}
      {/* CTA — Working newsletter signup              */}
      {/* ============================================ */}
      <section className="cta-section" id="subscribe">
        <div className="section-label" style={{ color: 'var(--copper)' }}>The Daily Ledge</div>
        <h2 className="cta-title">
          {totalArticles > 0 
            ? 'Get the weekly leadership brief.' 
            : 'The Daily Ledge launches soon.'}
        </h2>
        <p className="cta-text">
          A weekly digest of the sharpest leadership intelligence — curated by AI, 
          sharpened with perspective. Every link leads back to insights that matter.
        </p>
        
        {subscribeStatus === 'success' ? (
          <div className="subscribe-success">
            <div className="success-icon">✓</div>
            <p>Welcome to the Ledge.</p>
          ...
          <form className="cta-form" onSubmit={handleSubscribe}>
            ...
            <button
              ...
              disabled={subscribeStatus === 'sending'}
            >
              {subscribeStatus === 'sending' ? 'Joining...' : 'Join the Ledge'}
            </button>
          </form>
        )}
        
        {subscribeStatus === 'error' && (
          <p className="cta-error">Something went wrong. Please try again.</p>
        )}
      ...
      <footer>
        ...
        <div className="footer-text">© 2026 Ledge. All rights reserved.</div>
