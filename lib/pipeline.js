...
export async function runPipeline() {
  ...
  try {
    ...
    const stored = await storeRawArticles(supabase, newArticles);
    ...
    const relevant = classified.filter(c => c.classification.relevance >= 8);
    log.articles_passed = relevant.length;
    console.log(`${relevant.length} articles passed relevance threshold`);
